...
class StyleTransfer:
    ...
    def load_model(self, name, weights='imagenet'):
        ...
        self.conv_layers = [layer.name for layer in self.model.layers if isinstance(
    ...
    def __call__(self, content_img, style_img,
        ...
        layers_of_interest = list(
            set(content_layer_weights).union(style_layer_weights))

        # Create feature extractor.
        outputs = {layer_name: self.model.get_layer(
            layer_name).output for layer_name in layers_of_interest}
        self.feature_extractor = tf.keras.Model(self.model.inputs, outputs)

        # Get content and style features only once before training.
        # Not sure whether tf.constant() is important here.
        input_content_features = self.feature_extractor(
            tf.constant(content_prep))
        ...
        self.content_targets = StyleTransfer.build_content_layer_map(
            input_content_features, content_layer_weights.keys())

        # Map style layers to the gram matrices calculated for each layer of our style representation.
        self.style_targets = StyleTransfer.build_style_layer_map(
            input_style_features, style_layer_weights.keys())
        ...
        for step in range(1, steps+1):
            ...
            output_image_resized = tf.image.resize(
                output_image[0], size=content_img.shape[:2], method='bilinear')
    ...
    def __step(self, output_image):
        ...
        with tf.GradientTape() as tape:  # Record operations for automatic differentiation
            ...
            output_content_map = StyleTransfer.build_content_layer_map(
                output_features, content_layer_weights.keys())
            output_style_map = StyleTransfer.build_style_layer_map(
                output_features, style_layer_weights.keys())
            ...
            total_loss = self.content_reconstruction_weight * \
                content_loss + self.style_reconstruction_weight*style_loss
            ...
            if self.total_variation_weight > 0:
                ...
                variation_loss = tf.reduce_sum(
                    tf.abs(x_var)) + tf.reduce_sum(tf.abs(y_var))
        ...
        output_image.assign(tf.clip_by_value(
            output_image, clip_value_min=0.0, clip_value_max=255.0))
    ...
    def adjust_shape(image, size):
        ...
        """
        image_prep = tf.image.resize(
            image, size=size, method='lanczos5')  # resize appropriately
        # add the batch dimension and discard the alpha channel
    ...
    def build_content_layer_map(features, content_layers):
        ...
        """
        content_map = {layer_name: features[layer_name]
    ...
    def build_style_layer_map(features, style_layers):
        ...
        def gram_norm(f): return StyleTransfer.compute_gram_matrix(
...
def get_layer_weights(conv_layers, chosen_layers, layer_type):
    ...
    def layer_weight_func(layer_name, chosen_layers): return st.sidebar.slider(
...
try:
    ...
    content_path = st.sidebar.selectbox('Content image', options=glob.glob('data/*.*'),
    ...
    style_path = st.sidebar.selectbox('Style image', options=glob.glob('./sketches/*.*'),
                                      format_func=lambda glob_path: os.path.basename(glob_path))
    ...
    steps = st.sidebar.number_input(
        label='Steps', min_value=1, max_value=10000, value=20, step=1)

    # Choose the model.
    model_name = st.sidebar.selectbox(label='Model',
                                      options=[
                                          'VGG16', 'VGG19', 'Inception_V3', 'Xception', 'DenseNet', 'ResNet', 'ResNet_V2'],
                                      index=0)
    ...
    content_reconstruction_weight = st.sidebar.slider(
        label='Content reconstruction weight (alpha)', min_value=1, max_value=10000, value=1)
    style_reconstruction_weight = st.sidebar.slider(
        label='Style reconstruction weight (beta)', min_value=1, max_value=10000, value=1000)

    # A regularization term on the high frequency components of the image
    total_variation_weight = st.sidebar.slider(
        label='Total variation weight', min_value=0, max_value=100, value=12)
    ...
    content_layer_weights = get_layer_weights(
        conv_layers, conv_layers[-1:], 'content')
    ...
    style_layer_weights = get_layer_weights(
        conv_layers, conv_layers[:1], 'style')
    ...
    for step, output_image in style_transfer(content_img, style_img, steps, size,
        ...
        output_image_placeholder.image(
            output_image, caption='Output image', use_column_width=True, clamp=True, channels='RGB')
